...
{
    /// <summary>
    /// The build state used to track the progress
    /// </summary>
    public enum DungeonModelBuildState
    {
        Initial,
        Separation,
        Triangulation,
	...
	public class GridDungeonModelUtils {
		...
		public static Cell[] FindFurthestRooms(GridDungeonModel model) {
			...
			foreach (var startCell in model.Cells) {
				...
				var startData = new LongestPathBFSData { cell = startCell, length = 0 };
				...
				while (queue.Count > 0) {
					...
					foreach (var childId in front.cell.FixedRoomConnections) {
						...
						var childData = new LongestPathBFSData { cell = child, length = front.length + 1 };
