...
namespace DungeonArchitect.Builders.Isaac
{
    public class IsaacDungeonBuilder : DungeonBuilder
    {
        ...
        {
            var startPosition = IntVector.Zero;
            var queue = new Queue<LevelGrowthNode>();
            var visited = new HashSet<IntVector>();

            var roomFactory = new IsaacRoomFactory(isaacConfig.roomWidth, isaacConfig.roomHeight);
            rooms.Clear();
            doors.Clear();
    ...
    class IsaacRoomFactory
    {
        ...
        public IsaacRoomFactory(int roomWidth, int roomHeight)
        {
